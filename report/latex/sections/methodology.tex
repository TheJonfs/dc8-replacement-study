...
\begin{itemize}
    \item \textbf{Troposphere} (0 to 36,089~ft): Temperature decreases linearly at 3.566 degrees~F per 1,000~ft from a sea level value of 518.67~degrees~R.
    \item \textbf{Stratosphere} (36,089 to 65,000~ft): Temperature is constant at 389.97~degrees~R; pressure and density decrease exponentially.
\end{itemize}
...
where:
\begin{itemize}
    \item $C_{D_0}$ is the zero-lift drag coefficient (parasite drag from skin friction, form drag, and interference)
    \item $C_L$ is the lift coefficient, determined by the requirement that lift equals weight in steady cruise
    \item $AR$ is the wing aspect ratio (known from published geometry)
    \item $e$ is the Oswald efficiency factor (a calibration parameter capturing the deviation from ideal elliptical lift distribution)
\end{itemize}
...
\begin{equation}
    \mathit{TSFC}(h) = \mathit{TSFC}_{\text{ref}} \cdot k_{\text{adj}} \cdot \left(\frac{\rho(h)}{\rho_0}\right)^{-0.1}
    \label{eq:tsfc}
\end{equation}

where $\mathit{TSFC}_{\text{ref}}$ is the manufacturer-published cruise TSFC, $k_{\text{adj}}$ is a calibration factor, and the density ratio term captures the altitude dependence of fuel consumption in turbofan engines (TSFC generally increases modestly with altitude).

\subsubsection{Thrust Lapse Model}

Available thrust decreases with altitude as air density decreases. The model uses a two-regime lapse rate:

\textbf{Below the tropopause} ($h < 36{,}089$~ft):
\begin{equation}
    T = T_{SLS} \cdot \sigma^{0.75}
    \label{eq:thrust-tropo}
\end{equation}

\textbf{Above the tropopause} ($h \geq 36{,}089$~ft):
\begin{equation}
    T = T_{\text{trop}} \cdot \left(\frac{\sigma}{\sigma_{\text{trop}}}\right)^{2.0}
    \label{eq:thrust-strato}
\end{equation}
...
\subsubsection{Engine-Out Thrust}
...
\begin{enumerate}
    \item Compute optimal altitude for current weight (the altitude maximizing $L/D$ or specific range)
    \item Cap altitude at the aircraft's thrust-limited ceiling (where available thrust equals drag at that altitude)
    \item Compute range for the fuel burned in this step using Breguet
    \item Update weight and advance to the next step
\end{enumerate}
...
\subsubsection{Non-Cruise Fuel Overhead}
...
\subsection{Reserve Fuel Policy}

Two reserve fuel policies are used depending on mission type:

\textbf{Missions~2 and~3 (explicit reserves):}
\begin{itemize}
    \item 5\% of trip fuel as contingency
    \item 200~nm alternate airport capability
    \item 30 minutes holding fuel at destination altitude
\end{itemize}

\textbf{Mission~1 ($f_{oh}$ model):}
...
\subsection{Mission-Specific Models}

\subsubsection{Mission 1: Engine-Out Transport}

The mission is modeled as two consecutive cruise segments:

\begin{enumerate}
    \item \textbf{Normal cruise} (0 to 2,525~nm): All engines operating, step-cruise altitude optimization.
    \item \textbf{Engine-out cruise} (2,525~nm to fuel exhaustion): One engine failed, remaining engines at maximum continuous thrust, $+10\%$ drag penalty. The aircraft descends to its new thrust-limited ceiling and continues cruise.
\end{enumerate}
...
\subsubsection{Mission 2: Vertical Atmospheric Sampling}

The mission is modeled as a series of repeating climb-descend cycles:

\begin{enumerate}
    \item \textbf{Climb} from 5,000~ft to the aircraft's current thrust-limited ceiling, using an energy-method climb model that computes fuel, distance, and time as functions of rate-of-climb, which itself depends on excess thrust.
    \item \textbf{Descend} from ceiling back to 5,000~ft with reduced fuel consumption.
    \item \textbf{Repeat} until total distance reaches 4,200~nm.
\end{enumerate}
...
\subsubsection{Mission 3: Low-Altitude Smoke Survey}
...
\begin{enumerate}
    \item Estimate initial fuel requirement from clean-aircraft burn rate
    \item Compute takeoff weight (OEW $+$ payload $+$ fuel estimate)
    \item Simulate 8-hour endurance at that takeoff weight
    \item Compute required reserves for the fuel burned
    \item Iterate until fuel estimate converges within 50~lb
    \item Add 5\% operational margin
\end{enumerate}
