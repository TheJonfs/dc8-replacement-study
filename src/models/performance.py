...
def generate_range_payload_curve(aircraft_data, CD0, e, k_adj=1.0,
                                  n_payload_points=30, n_steps=50):
    """Generate the full range-payload curve.

    Traces the standard three-segment range-payload diagram:
    Segment 1 (A→B): Max payload, increasing fuel up to MTOW or tank limit
    Segment 2 (B→C): Max fuel, decreasing payload from MTOW limit to zero
    Segment 3 endpoint: Zero payload, max fuel (ferry)

    Actually generates a smooth curve by varying payload from max to zero,
    computing the appropriate fuel for each case.
    ...
    Args:
        ...
        e: Oswald span efficiency factor (calibrated)
        k_adj: TSFC adjustment factor (calibrated)
        n_payload_points: Number of points along the curve
        n_steps: Number of cruise steps for each point

    Returns:
        List of (payload_lb, range_nm, fuel_lb, takeoff_weight_lb) tuples
    """
    ac = aircraft_data
    curve = []

    # Generate payload values from max_payload down to 0
    payloads = []
    max_pl = ac["max_payload"]
    for i in range(n_payload_points + 1):
        pl = max_pl * (1.0 - i / n_payload_points)
        payloads.append(pl)

    for payload in payloads:
        # Determine fuel for this payload
        # Fuel is limited by: (1) tank capacity, (2) MTOW constraint
        fuel_mtow_limit = ac["MTOW"] - ac["OEW"] - payload
        fuel = min(fuel_mtow_limit, ac["max_fuel"])
        fuel = max(fuel, 0)  # can't have negative fuel

        if fuel <= 0:
            curve.append((payload, 0.0, 0.0, ac["OEW"] + payload))
            continue

        try:
            result = compute_range_for_payload(
                ac, payload, fuel, CD0, e, k_adj, n_steps
            )
            tow = ac["OEW"] + payload + fuel
            curve.append((payload, result["range_nm"], fuel, tow))
        except Exception:
            curve.append((payload, 0.0, fuel, ac["OEW"] + payload + fuel))
